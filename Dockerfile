...
COPY build/docker/default.conf /etc/nginx/http.d/default.conf
COPY build/docker/www.conf /usr/local/etc/php-fpm.conf
COPY build/docker/supervisord.conf /etc/supervisord.conf

EXPOSE 80 443
ENTRYPOINT [ "/bin/ash", "build/docker/entrypoint.sh" ]
